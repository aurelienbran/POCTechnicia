"""
Implémentation du service OCR utilisant directement Tesseract sans OCRmyPDF.
Ce service peut être utilisé comme alternative quand OCRmyPDF n'est pas disponible.
"""
...
class TesseractDirectProcessor(OCRProcessor):
    """Service OCR utilisant directement Tesseract via pdf2image."""
    
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        """
        Initialise le processeur OCR avec Tesseract direct.
        ...
        self.temp_dir = self.config.get('temp_dir') or Path(tempfile.gettempdir()) / "technicia_ocr"
    ...
    def provider_name(self) -> str:
        ...
        Returns:
            "tesseract_direct"
        """
        return "tesseract_direct"
    
    @retry(reraise=True, stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=2, max=10))
    async def initialize(self) -> bool:
        """
        Vérifie que Tesseract et ses dépendances sont correctement installés.
        ...
        try:
            ...
            logger.error(f"Erreur lors de l'initialisation de Tesseract: {str(e)}")
    ...
    async def process_document(self, 
        ...
        # Répertoire temporaire pour les images et textes
        work_dir = Path(self.temp_dir) / f"work_{int(time.time())}"
        os.makedirs(work_dir, exist_ok=True)
        
        try:
            # Vérifier si OCR est nécessaire
            needs_ocr = True
            if not kwargs.get("force_ocr", False) and input_path.suffix.lower() == '.pdf':
                needs_ocr = await self.needs_ocr(input_path)
            
            # Si OCR n'est pas nécessaire, copier simplement le fichier d'entrée
            if not needs_ocr and input_path.suffix.lower() == '.pdf':
                shutil.copy(input_path, output_path)
                logger.info(f"OCR non nécessaire, fichier copié: {input_path} -> {output_path}")
            ...
            logger.info(f"Conversion du PDF en images: {input_path}")
            ...
            logger.info(f"PDF converti en {total_pages} images")
            
            # Traiter chaque image avec Tesseract
            text_files = []
            for i, img_path in enumerate(images):
                logger.info(f"OCR page {i+1}/{total_pages}")
                
                out_base = work_dir / f"page_{i+1}"
                
                # Appliquer Tesseract avec sortie PDF
                cmd = [
                    "tesseract", 
                    str(img_path),
                    str(out_base),
                    "-l", language,
                    "pdf", "txt"  # Générer PDF et TXT
                ]
                
                await self._run_command(cmd)
                
                # Ajouter les fichiers générés à la liste
                text_files.append(str(out_base) + ".txt")
            
            # Fusionner tous les PDFs générés
            pdf_files = [str(work_dir / f"page_{i+1}.pdf") for i in range(total_pages)]
            
            # Utiliser PyMuPDF pour fusionner les PDFs
            doc = fitz.open()
            ...
            logger.error(f"Erreur lors du traitement OCR direct: {str(e)}")
    ...
    async def extract_text(self, 
        ...
        try:
            ...
            for page_num in pages_to_extract:
                ...
                # Si la page ne contient pas assez de texte, on peut essayer d'extraire 
                # avec Tesseract directement depuis l'image
                if len(page_text.strip()) < 50:
                    # La page pourrait avoir besoin d'un OCR plus poussé
                    # Cette logique pourrait être implémentée pour améliorer l'extraction
                    pass
